...
local getfontsize = nexo.Libraries.getfontsize
...
local function canClick()
	...
	return (not nexo.gui.ScaledGui.ClickGui.Visible) and (not inputService:GetFocusedTextBox())
end
...
local function serverHop(pointer, filter)
	visited = shared.nexoserverhoplist and shared.nexoserverhoplist:split('/') or {}
	if not table.find(visited, game.JobId) then
		table.insert(visited, game.JobId)
	end
	if not pointer then
		notif('nexo', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('nexo', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('nexo', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('nexo', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end

nexo:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport("shared.nexoserverhoplist = '"..table.concat(visited, '/').."'\nshared.nexoserverhopprevious = '"..game.JobId.."'")
	end
end))
...
local hash = loadstring(downloadFile('nexo/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('nexo/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('nexo/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
...
nexo.Libraries.entity = entitylib
nexo.Libraries.whitelist = whitelist
nexo.Libraries.prediction = prediction
nexo.Libraries.hash = hash
nexo.Libraries.auraanims = {
	Normal = {
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		if nexo.Categories.Main.Options['Teams by server'].Enabled then
	...
	nexo:Clean(nexo.Categories.Friends.Update.Event:Connect(function() entitylib.refresh() end))
	nexo:Clean(nexo.Categories.Targets.Update.Event:Connect(function() entitylib.refresh() end))
	nexo:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
	nexo:Clean(workspace:GetPropertyChangedSignal('CurrentCamera'):Connect(function()
...
run(function()
	...
	function whitelist:playeradded(v, joined)
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				olduninject = nexo.Uninject
				nexo.Uninject = function()
					notif('nexo', 'No escaping the private members :)', 10)
				end
				if joined then
					task.wait(10)
				end
	...
	function whitelist:process(msg, plr)
		...
		if self.localprio > 0 and not self.said[plr.Name] and msg == 'helloimusinginhaler' and plr ~= lplr then
			...
			notif('nexo', plr.Name..' is using nexo!', 60)
	...
	function whitelist:hook()
		...
		if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
			if exp and exp:WaitForChild('appLayout', 5) then
				nexo:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
		...
		if exp then
			...
			if bubblechat then
				nexo:Clean(bubblechat.DescendantAdded:Connect(function(newbubble)
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('nexo/profiles/whitelist.json') and readfile('nexo/profiles/whitelist.json') or nil
			end
			...
			if not whitelist.connection then
				...
				nexo:Clean(whitelist.connection)
			end
			...
			if entitylib.Running and nexo.Loaded then
				entitylib.refresh()
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				if whitelist.data.Announcement.expiretime > os.time() then
					...
					if table.find(targets, tostring(lplr.UserId)) then
						...
						hint.Text = 'nexo ANNOUNCEMENT: '..whitelist.data.Announcement.text
				...
				pcall(function()
					writefile('nexo/profiles/whitelist.json', whitelist.textdata)
				end)
			end

			if whitelist.data.Killnexo then
				nexo:Uninject()
				return true
			end
			...
			task.spawn(function()
				...
				task.delay(0.6, function()
					local modPrompt = Roact.createElement(auth, {
						style = {},
						screenSize = nexo.gui.AbsoluteSize or Vector2.new(1920, 1080),
						moderationDetails = {
		...
		Function = function(callback)
			...
			if callback then
				...
				AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
					...
					if rightClicked and not nexo.gui.ScaledGui.ClickGui.Visible then
				...
				repeat
					if Mode.Value == 'Tool' then
						...
						if mouse1click and (isrbxactive or iswindowactive)() then
							if not nexo.gui.ScaledGui.ClickGui.Visible then
...
run(function()
	...
	Reach = nexo.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	TriggerBot = nexo.Categories.Combat:CreateModule({
		Name = 'TriggerBot',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Fly = nexo.Categories.Blatant:CreateModule({
		Name = 'Fly',
		Function = function(callback)
			if Platform then
...
run(function()
	...
	HighJump = nexo.Categories.Blatant:CreateModule({
		Name = 'HighJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitBoxes = nexo.Categories.Blatant:CreateModule({
		Name = 'HitBoxes',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Invisible = nexo.Categories.Blatant:CreateModule({
		Name = 'Invisible',
		Function = function(callback)
			if callback then
				if not proper then
...
run(function()
	...
	LongJump = nexo.Categories.Blatant:CreateModule({
		Name = 'LongJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MouseTP = nexo.Categories.Blatant:CreateModule({
		Name = 'MouseTP',
		Function = function(callback)
			if callback then
				local position
...
run(function()
	...
	Speed = nexo.Categories.Blatant:CreateModule({
		Name = 'Speed',
		Function = function(callback)
...
run(function()
	...
	Spider = nexo.Categories.Blatant:CreateModule({
		Name = 'Spider',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Swim = nexo.Categories.Blatant:CreateModule({
		Name = 'Swim',
		Function = function(callback)
			if callback then
...
run(function()
	...
	TargetStrafe = nexo.Categories.Blatant:CreateModule({
		Name = 'TargetStrafe',
		Function = function(callback)
			if callback then
				if not module then
				...
				local flymod, ang, oldent = nexo.Modules.Fly or {Enabled = false}
...
run(function()
	...
	Timer = nexo.Categories.Blatant:CreateModule({
		Name = 'Timer',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('nexo/assets/new/arrowmodule.png')
	...
	Arrows = nexo.Categories.Render:CreateModule({
		Name = 'Arrows',
		Function = function(callback)
			if callback then
				...
				Arrows:Clean(nexo.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Chams = nexo.Categories.Render:CreateModule({
		Name = 'Chams',
		Function = function(callback)
			if callback then
				...
				Chams:Clean(nexo.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	ESP = nexo.Categories.Render:CreateModule({
		Name = 'ESP',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					ESP:Clean(nexo.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	GamingChair = nexo.Categories.Render:CreateModule({
		Name = 'GamingChair',
		Function = function(callback)
			if callback then
				if nexo.ThreadFix then
				...
				--movingsound.SoundId = downloadnexoAsset('nexo/assets/ChairRolling.mp3')
				movingsound.Volume = 0.4
				movingsound.Looped = true
				movingsound.Parent = workspace
				flyingsound = Instance.new('Sound')
				--flyingsound.SoundId = downloadnexoAsset('nexo/assets/ChairFlying.mp3')
				...
				repeat
					if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 then
						...
						local flying = nexo.Modules.Fly and nexo.Modules.Fly.Enabled or nexo.Modules.LongJump and nexo.Modules.LongJump.Enabled or nexo.Modules.InfiniteFly and nexo.Modules.InfiniteFly.Enabled
...
run(function()
	...
	Health = nexo.Categories.Render:CreateModule({
		Name = 'Health',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NameTags = nexo.Categories.Render:CreateModule({
		Name = 'NameTags',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					NameTags:Clean(nexo.Categories.Friends.ColorUpdate.Event:Connect(function()
						ColorFunc[methodused](Color.Hue, Color.Sat, Color.Value)
					end))
				end
...
run(function()
	...
	PlayerModel = nexo.Categories.Render:CreateModule({
		Name = 'PlayerModel',
		Function = function(callback)
			if callback then 
...
run(function()
	...
	Radar = nexo:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('nexo/assets/new/radaricon.png'),
		...
		Function = function(callback)
			if callback then
				...
				Radar:Clean(nexo.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Search = nexo.Categories.Render:CreateModule({
		Name = 'Search',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SessionInfo = nexo:CreateOverlay({
		Name = 'Session Info',
			...
			if callback then
				...
				SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
					if not teleportedServers then
						...
						queue_on_teleport("shared.nexosessioninfo = '"..httpService:JSONEncode(nexo.Libraries.sessioninfo.Objects).."'")
					end
				end))
	
				if shared.nexosessioninfo then
					for i, v in httpService:JSONDecode(shared.nexosessioninfo) do
						if nexo.Libraries.sessioninfo.Objects[i] and v.Saved then
							nexo.Libraries.sessioninfo.Objects[i].Value = v.Value
						end
					end
				end
	
				repeat
					if nexo.Libraries.sessioninfo then
						local stuff = {''}
						if Title.Enabled then
							stuff[1] = TitleOffset.Enabled and '<b>Session Info</b>\n<font size="4"> </font>' or '<b>Session Info</b>'
						end
	
						for i, v in nexo.Libraries.sessioninfo.Objects do
	...
	nexo:Clean(SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
		if nexo.ThreadFix then
			setthreadidentity(8)
		end
		local newside = SessionInfo.Children.AbsolutePosition.X > (nexo.gui.AbsoluteSize.X / 2)
	...
	nexo.Libraries.sessioninfo:AddItem('Time Played', os.clock(), function(value)
...
run(function()
	...
	local function Loop()
		local screenSize = nexo.gui.AbsoluteSize
	...
	Tracers = nexo.Categories.Render:CreateModule({
		Name = 'Tracers',
		Function = function(callback)
			if callback then
				...
				Tracers:Clean(nexo.Categories.Friends.ColorUpdate.Event:Connect(function()
					ColorFunc(Color.Hue, Color.Sat, Color.Value)
				end))
...
run(function()
	...
	WaypointFolder.Parent = nexo.gui
	
	Waypoints = nexo.Categories.Render:CreateModule({
		Name = 'Waypoints',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AnimationPlayer = nexo.Categories.Utility:CreateModule({
...
run(function()
	...
	AntiRagdoll = nexo.Categories.Utility:CreateModule({
		Name = 'AntiRagdoll',
		Function = function(callback)
			if entitylib.isAlive then
...
run(function()
	...
	AutoRejoin = nexo.Categories.Utility:CreateModule({
		Name = 'AutoRejoin',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	Blink = nexo.Categories.Utility:CreateModule({
		Name = 'Blink',
		Function = function(callback)
			if callback then
				local teleported
...
run(function()
	...
	Disabler = nexo.Categories.Utility:CreateModule({
		Name = 'Disabler',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Rejoin = nexo.Categories.Utility:CreateModule({
		Name = 'Rejoin',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ServerHop = nexo.Categories.Utility:CreateModule({
		Name = 'ServerHop',
		Function = function(callback)
			if callback then
				ServerHop:Toggle()
	...
	ServerHop:CreateButton({
		...
		Function = function()
			notif('ServerHop', shared.nexoserverhopprevious and 'Rejoining previous server...' or 'Cannot find previous server', 5)
			if shared.nexoserverhopprevious then
				teleportService:TeleportToPlaceInstance(game.PlaceId, shared.nexoserverhopprevious)
			end
		end
	})
end)
...
run(function()
	...
	local function playerAdded(plr)
		if not nexo.Loaded then
			repeat task.wait() until nexo.Loaded
		end
		...
		if user or getRole(plr, tonumber(Group.Value) or 0) >= (tonumber(Role.Value) or 1) then
			...
			elseif Mode.Value == 'Profile' then
				nexo.Save = function() end
				if nexo.Profile ~= Profile.Value then
					nexo.Profile = Profile.Value
					nexo:Load(true, Profile.Value)
				end
	...
	StaffDetector = nexo.Categories.Utility:CreateModule({
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	nexo.Categories.World:CreateModule({
		Name = 'Anti-AFK',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Freecam = nexo.Categories.World:CreateModule({
		Name = 'Freecam',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Gravity = nexo.Categories.World:CreateModule({
		Name = 'Gravity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Parkour = nexo.Categories.World:CreateModule({
		Name = 'Parkour',
		Function = function(callback)
			if callback then 
				local oldfloor
...
run(function()
	...
	Xray = nexo.Categories.World:CreateModule({
		Name = 'Xray',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MurderMystery = nexo.Categories.Minigames:CreateModule({
		Name = 'MurderMystery',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Atmosphere = nexo.Legit:CreateModule({
		Name = 'Atmosphere',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Breadcrumbs = nexo.Legit:CreateModule({
		Name = 'Breadcrumbs',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ChinaHat = nexo.Legit:CreateModule({
		Name = 'China Hat',
		Function = function(callback)
			if callback then
				if nexo.ThreadFix then
...
run(function()
	...
	Disguise = nexo.Legit:CreateModule({
		Name = 'Disguise',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Keystrokes = nexo.Legit:CreateModule({
		Name = 'Keystrokes',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Memory = nexo.Legit:CreateModule({
		Name = 'Memory',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SongBeats = nexo.Legit:CreateModule({
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Speedmeter = nexo.Legit:CreateModule({
		Name = 'Speedmeter',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	TimeChanger = nexo.Legit:CreateModule({
		Name = 'Time Changer',
		Function = function(callback)
			if callback then
