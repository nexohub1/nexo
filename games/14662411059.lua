...
nexo.Place = 11630038968
if isfile('nexo/games/'..nexo.Place..'.lua') then
	loadstring(readfile('nexo/games/'..nexo.Place..'.lua'), 'bridge duel')()
else
	if not shared.nexoDeveloper then
		local suc, res = pcall(function() return 
			game:HttpGet('https://raw.githubusercontent.com/nexohub1/nexo/'..readfile('nexo/profiles/commit.txt')..'/games/'..nexo.Place..'.lua', true) 
		end)
		if suc and res ~= '404: Not Found' then
			loadstring(downloadFile('nexo/games/'..nexo.Place..'.lua'), 'bridge duel')()
		end
	end
end
