...
local function hotbarSwitch(slot)
	if slot and store.inventory.hotbarSlot ~= slot then
		...
		nexoEvents.InventoryChanged.Event:Wait()
		return true
	end
	return false
end
			...
			if hum and humrootpart then
				...
				if plr == lplr then
					...
					table.insert(entitylib.Connections, char.AttributeChanged:Connect(function(attr)
						nexoEvents.AttributeChanged:Fire(attr)
					end))
				else
...
run(function()
	...
	for i, v in remoteNames do
		...
		if remote == '' then
			notif('nexo', 'Failed to grab remote ('..i..')', 10, 'alert')
		end
		remotes[i] = remote
	end
	...
	local function updateStore(new, old)
		...
		if new.Inventory ~= old.Inventory then
			...
			if newinv ~= oldinv then
				nexoEvents.InventoryChanged:Fire()
			end
	...
	for _, event in {'MatchEndEvent', 'EntityDeathEvent', 'BedwarsBedBreak', 'BalloonPopped', 'AngelProgress', 'GrapplingHookFunctions'} do
		if not nexo.Connections then return end
		bedwars.Client:WaitFor(event):andThen(function(connection)
			nexo:Clean(connection:Connect(function(...)
				nexoEvents[event]:Fire(...)
			end))
		end)
	end

	nexo:Clean(bedwars.ZapNetworking.EntityDamageEventZap.On(function(...)
		nexoEvents.EntityDamageEvent:Fire({
			entityInstance = ...,
			damage = select(2, ...),
			damageType = select(3, ...),
	...
	for _, event in {'PlaceBlockEvent', 'BreakBlockEvent'} do
		nexo:Clean(bedwars.ZapNetworking[event..'Zap'].On(function(...)
			local data = {
				blockRef = {
					blockPosition = ...,
				},
	...
	task.spawn(function()
		pcall(function()
			repeat task.wait() until store.matchState ~= 0 or nexo.Loaded == nil
	...
	nexo:Clean(nexoEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
		if bedTable.player and bedTable.player.UserId == lplr.UserId then
			beds:Increment()
		end
	end))

	nexo:Clean(nexoEvents.MatchEndEvent.Event:Connect(function(winTable)
		if (bedwars.Store:getState().Game.myTeam or {}).id == winTable.winningTeamId or lplr.Neutral then
			wins:Increment()
		end
	end))

	nexo:Clean(nexoEvents.EntityDeathEvent.Event:Connect(function(deathTable)
	...
	pcall(function()
		if getthreadidentity and setthreadidentity then
			...
			task.spawn(function()
				repeat
					task.wait(0.1)
				until nexo.Loaded == nil or bedwars.AppController:isAppOpen('BedwarsItemShopApp')
...
run(function()
	...
	AutoCharge = nexo.Categories.Combat:CreateModule({
	    Name = 'AutoCharge',
	    Function = function(callback)
...
run(function()
	...
	nexo.Categories.Combat:CreateModule({
		Name = 'NoClickDelay',
		Function = function(callback)
			if callback then
							...
							if Mode.Value == 'Normal' then
								...
								if top then
									...
									connection = runService.PreSimulation:Connect(function()
										if nexo.Modules.Fly.Enabled or nexo.Modules.InfiniteFly.Enabled or nexo.Modules.LongJump.Enabled then
...
run(function()
	...
	FastBreak = nexo.Categories.Blatant:CreateModule({
		Name = 'FastBreak',
		Function = function(callback)
			if callback then
				repeat
	...
	Fly = nexo.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			if callback then
				...
				Fly:Clean(nexoEvents.AttributeChanged.Event:Connect(function(changed)
...
run(function()
	...
	local anims, AnimDelay, AnimTween, armC0 = nexo.Libraries.auraanims, tick()
...
run(function()
	...
	local LongJumpMethods = {
		...
		cat = function(_, _, dir)
			LongJump:Clean(nexoEvents.CatPounce.Event:Connect(function()
	...
	LongJump = nexo.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			updateVelocity()
			if callback then
				LongJump:Clean(nexoEvents.EntityDamageEvent.Event:Connect(function(damageTable)
				...
				LongJump:Clean(nexoEvents.GrapplingHookFunctions.Event:Connect(function(dataTable)
...
run(function()
	...
	BedESP = nexo.Categories.Render:CreateModule({
		Name = 'BedESP',
		Function = function(callback)
			if callback then
	...
	Health = nexo.Categories.Render:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				Health:Clean(label)
				Health:Clean(nexoEvents.AttributeChanged.Event:Connect(function()
					label.Text = entitylib.isAlive and math.round(lplr.Character:GetAttribute('Health'))..' ❤️' or ''
					label.TextColor3 = entitylib.isAlive and Color3.fromHSV((lplr.Character:GetAttribute('Health') / lplr.Character:GetAttribute('MaxHealth')) / 2.8, 0.86, 1) or Color3.new()
				end))
			end
...
run(function()
	...
	KitESP = nexo.Categories.Render:CreateModule({
		Name = 'KitESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	StorageESP = nexo.Categories.Render:CreateModule({
		Name = 'StorageESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBalloon = nexo.Categories.Utility:CreateModule({
		Name = 'AutoBalloon',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoKit = nexo.Categories.Utility:CreateModule({
		Name = 'AutoKit',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPearl = nexo.Categories.Utility:CreateModule({
		Name = 'AutoPearl',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	AutoPlay = nexo.Categories.Utility:CreateModule({
		Name = 'AutoPlay',
		Function = function(callback)
			if callback then
				AutoPlay:Clean(nexoEvents.EntityDeathEvent.Event:Connect(function(deathTable)
					if deathTable.finalKill and deathTable.entityInstance == lplr.Character and isEveryoneDead() and store.matchState ~= 2 then
						joinQueue()
					end
				end))
				AutoPlay:Clean(nexoEvents.MatchEndEvent.Event:Connect(joinQueue))
			end
		end,
...
run(function()
	...
	AutoToxic = nexo.Categories.Utility:CreateModule({
		Name = 'AutoToxic',
		Function = function(callback)
			if callback then
				AutoToxic:Clean(nexoEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
				...
				AutoToxic:Clean(nexoEvents.EntityDeathEvent.Event:Connect(function(deathTable)
				...
				AutoToxic:Clean(nexoEvents.MatchEndEvent.Event:Connect(function(winstuff)
					if GG.Enabled then
						if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
...
run(function()
	...
	AutoVoidDrop = nexo.Categories.Utility:CreateModule({
		Name = 'AutoVoidDrop',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MissileTP = nexo.Categories.Utility:CreateModule({
		Name = 'MissileTP',
		Function = function(callback)
			if callback then
				MissileTP:Toggle()
...
run(function()
	...
	RavenTP = nexo.Categories.Utility:CreateModule({
		Name = 'RavenTP',
		Function = function(callback)
			if callback then
				RavenTP:Toggle()
...
run(function()
	...
	ShopTierBypass = nexo.Categories.Utility:CreateModule({
		Name = 'ShopTierBypass',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function playerAdded(plr)
		...
		elseif getRole(plr, 5774246) >= 100 then
			...
			if table.find(blacklistedclans, plr:GetAttribute('ClanTag')) and nexo.Loaded and Clans.Enabled then
				connection:Disconnect()
	...
	task.spawn(function()
		repeat task.wait(1) until nexo.Loaded or nexo.Loaded == nil
		if nexo.Loaded and not StaffDetector.Enabled then
...
run(function()
	TrapDisabler = nexo.Categories.Utility:CreateModule({
		Name = 'TrapDisabler',
...
run(function()
	...
	AutoSuffocate = nexo.Categories.World:CreateModule({
		Name = 'AutoSuffocate',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoTool = nexo.Categories.World:CreateModule({
		Name = 'AutoTool',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedProtector = nexo.Categories.World:CreateModule({
		Name = 'BedProtector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Schematica = nexo.Categories.World:CreateModule({
		Name = 'Schematica',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ArmorSwitch = nexo.Categories.Inventory:CreateModule({
		Name = 'ArmorSwitch',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBank = nexo.Categories.Inventory:CreateModule({
		Name = 'AutoBank',
		Function = function(callback)
			if callback then
	...
	AutoBuy = nexo.Categories.Inventory:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				local lastupgrades
				AutoBuy:Clean(nexoEvents.InventoryAmountChanged.Event:Connect(function()
	...
	AutoConsume = nexo.Categories.Inventory:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				AutoConsume:Clean(nexoEvents.AttributeChanged.Event:Connect(function(attribute)
...
run(function()
	...
	local function CreateWindow(self)
		...
		window.Parent = nexo.gui.ScaledGui
		...
		close.MouseButton1Click:Connect(function()
			window.Visible = false
			nexo.gui.ScaledGui.ClickGui.Visible = true
		end)
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			children.CanvasSize = UDim2.fromOffset(0, windowlist.AbsoluteContentSize.Y / nexo.guiscale.Scale)
		end)
		table.insert(nexo.Windows, window)
	...
	nexo.Components.HotbarList = function(optionsettings, children, api)
		if nexo.ThreadFix then
			setthreadidentity(8)
		end
		local optionapi = {
		...
		textbuttonicon.Image = getcustomasset('nexo/assets/new/add.png')
		...
		windowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
			...
			hotbarlist.Size = UDim2.fromOffset(220, math.min(43 + windowlist.AbsoluteContentSize.Y / nexo.guiscale.Scale, 603))
		end)
		...
		function optionapi:AddHotbar(data)
			...
			hotbar.MouseButton1Click:Connect(function()
				...
				if ind == optionapi.Selected then
					nexo.gui.ScaledGui.ClickGui.Visible = false
	...
	AutoHotbar = nexo.Categories.Inventory:CreateModule({
		Name = 'AutoHotbar',
		Function = function(callback)
			if callback then
				...
				AutoHotbar:Clean(nexoEvents.InventoryAmountChanged.Event:Connect(sortCallback))
			end
		end,
...
run(function()
	...
	local FastConsume = nexo.Categories.Inventory:CreateModule({
		Name = 'FastConsume',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastDrop = nexo.Categories.Inventory:CreateModule({
		Name = 'FastDrop',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	BedPlates = nexo.Categories.Minigames:CreateModule({
		Name = 'BedPlates',
		Function = function(callback)
			if callback then
				for _, v in collectionService:GetTagged('bed') do 
				...
				BedPlates:Clean(nexoEvents.PlaceBlockEvent.Event:Connect(refreshNear))
				BedPlates:Clean(nexoEvents.BreakBlockEvent.Event:Connect(refreshNear))
...
run(function()
	...
	BedBreakEffect = nexo.Legit:CreateModule({
		Name = 'Bed Break Effect',
		Function = function(callback)
			if callback then
	            BedBreakEffect:Clean(nexoEvents.BedwarsBedBreak.Event:Connect(function(data)
...
run(function()
	...
	local Crosshair = nexo.Legit:CreateModule({
		Name = 'Crosshair',
		Function = function(callback)
			if callback then
...
run(function()
	...
	DamageIndicator = nexo.Legit:CreateModule({
...
run(function()
	...
	FPSBoost = nexo.Legit:CreateModule({
		Name = 'FPS Boost',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitColor = nexo.Legit:CreateModule({
		Name = 'Hit Color',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	Interface = nexo.Legit:CreateModule({
		Name = 'Interface',
		Function = function(callback)
...
run(function()
	...
	KillEffect = nexo.Legit:CreateModule({
		Name = 'Kill Effect',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ReachDisplay = nexo.Legit:CreateModule({
		Name = 'Reach Display',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SoundChanger = nexo.Legit:CreateModule({
		Name = 'SoundChanger',
		Function = function(callback)
			if callback then
...
run(function()
	...
	UICleanup = nexo.Legit:CreateModule({
		Name = 'UI Cleanup',
		Function = function(callback)
			for i, v in (callback and new or old) do
				for i2, v2 in v do
...
run(function()
	...
	WinEffect = nexo.Legit:CreateModule({
		Name = 'WinEffect',
		Function = function(callback)
			if callback then
				WinEffect:Clean(nexoEvents.MatchEndEvent.Event:Connect(function()
